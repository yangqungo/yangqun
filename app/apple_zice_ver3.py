...
class Ctx:
    ...
    n: int = 1
    tid: int = 6005
    tids: list[int] = field(default_factory=lambda: [6005,  6014, 6015])
    tidx: int = 0
    r: int = 0
    td: int = 0
    ...
    bx: dict = field(default_factory=lambda: {"box_task_id": 6017, "llsid": 1, "creativeId": "a", "idempotentId": "", "posId": "", "taskToken": "", "task_step": 1, "task_status": 1})
...
def rf(c: C, ctx: Ctx, l: logging.LoggerAdapter) -> Ctx:
    ...
    try:
        if c.se.is_set():
            return ctx
        bp = bbp(ctx)
        gb = c.ar("/apple_xf_box", payload=bp)
        b = b64d(gb["body"])
        ...
        if ctx.bx.get("task_status") == 1 and not c.se.is_set():
            bop = bbp(ctx)
            gbo = c.ar("/apple_xf_box", payload=bop)
            b = b64d(gbo["body"])
            ...
            for _ in range(4):
                if ctx.bx.get("task_status") != 1 or c.se.is_set():
                    break
                gi = c.ar("/apple_xf_ad_inspire", bip(ctx))
                b = b64d(gi["body"])
    ...
    for _ in range(6):
        ...
        try:
            ...
            if c.se.wait(30):
                break
            gr = c.ar("/apple_xf_reward", btrp(ctx))
            b = b64d(gr["body"])
            ...
            if isinstance(ctx.rw, dict) and ctx.rw:
                ctx.td += 1
                l.info("%d 奖励结果: %s (累计=%d)", ct, ctx.rw, ctx.td)
